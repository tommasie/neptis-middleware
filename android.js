                ...
                function(callback) {
                    ...
                    request({
                        ...
                        if (!error & response.statusCode === 200) {
                            ...
                            if(body.length === 0) {
                                cost = 10;
                                Object.keys(attractionsArray).forEach(function(id){
                                ...
                                for (let i = 0; i < body.length; i++) {
                                    ...
                                    cost = parseInt(Math.round(parseFloat(body[i].value)),10);
                                    for (let j = 0; j < visits; j++) {
                                        string += "(:action visit-v" + j + "-" + id + "\n\t\t";
                                        string += ":precondition (and (cur_state top_" + id + ") (cur_state v" + j + ") (not (visited att_" + id + ")))\n\t\t";
                                        string += ":effect (and (cur_state v" + (j + 1) + ") (not (cur_state v" + j + ")) (visited att_" + id +") (increase (total-cost) " + cost +"))\n\t)\n\t";
    ...
    request({
        ...
        if (!error & response.statusCode === 200) {
            ...
            request({
                ...
                logger.debug(adjacencies);
                //Write problem.pddl
                problemData = "(define (problem Visit) (:domain Museum)\n\t(:objects\n\t\tstart ";

                for (let i = 0; i < roooms.length; i++) {
                    var areaName = roooms[i].areaM.name.toLowerCase().replace(/ /g, '_');
                    if(roomsName2Id[areaName] === undefined)
                        roomsName2Id[areaName] = roooms[i].areaM;
                    if(rooms[areaName] === undefined)
                        rooms[areaName] = [];
                    rooms[areaName].push(roooms[i].id);

                    roomIds[roooms[i].areaM.id] = areaName;
                }
                    ...
                    function(callback) {
                        ...
                        request({
                            ...
                            if (!error & response.statusCode === 200) {
                                ...
                                if(body.length === 0) {
                                    cost = 10;
                                    Object.keys(attractionsMap).forEach(function(id){
                                        for (let j = 0; j < visits; j++) {
                                            string += "(:action visit-v" + j + "-" + id + "\n\t\t";
